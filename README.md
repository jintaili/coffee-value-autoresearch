...
python scripts/backfill_jev.py pilot
python scripts/backfill_jev.py full
python scripts/evaluate_jev.py
```

The shared question catalog and encoder are in `coffee_value/extraction/`.
Keep credentials out of Git; `.env.example` lists the environment variable.
